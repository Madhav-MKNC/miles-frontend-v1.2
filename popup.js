...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (dict.toggleState) {
    ...
    console.log('dict.toggleState was true');
  }
  document.getElementById('toggleInput').addEventListener('change', function () {
    ...
    console.log("toggleState:", dict.toggleState);
  });
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    chrome.tabs.sendMessage(tabs[0].id, { geth3: true }, function (response) {
      name = response.otherUser;
      document.querySelector('h3').innerText = name; // display chat title
      if (name !== "" && dict.conversation_data[name]) {
        const placeholderValue = dict.conversation_data[name][0]; // get goal
        document.getElementById('myInput').placeholder = placeholderValue; // display goal
      }
      else {
